...
public class DasFenster extends JFrame implements ActionListener, FocusListener {  //
    ...
    */
    ...
    private void DasFensterMeth()
    {
        ...
        label1 = new JLabel("                   " + lC.gT("Ihr Text") + "               ");
        label1.setFont(schriftart); //Schriftart des Panels wird gesetzt

        label2 = new JLabel("      +     " + lC.gT("Ihr Passwort") + "                 ");
        label2.setFont(schriftart);

        label3 = new JLabel(lC.gT("Ihr Ergebnis") + "         ");
        label3.setFont(schriftart);

        platzhalter1 = new JLabel("");

        Eingabe = new JTextArea(lC.gT("Eingabetext"));  //  Eingabe wird mit Standarttext initialisiert
        ...
        Passwort = new JTextField(lC.gT("Passwort"), 20); //Passwort wird mit Standarttext und Spaltenbreite initialisiert
        ...
        ErgebnisFeld = new JTextArea(lC.gT("Ausgabe Feld"));
        ...
        ErgebnisFeld.setLineWrap(true);
        ErgebnisFeld.setWrapStyleWord(true);

        buttonEncrypt = new JButton(lC.gT("Verschlüsseln")); //Button wird mit Aufgesetzem Text initialisiert
        ...
        buttonDecrypt = new JButton(lC.gT("Entschlüsseln"));
        buttonDecrypt.setFont(schriftart3);
        buttonDecrypt.addActionListener(this);

        buttonExit = new JButton(lC.gT("Verlassen"));
        buttonExit.setFont(schriftart3);
        buttonExit.addActionListener(this);

        buttonPasswort = new JButton(lC.gT("Passwort generieren"));
    ...
    public void actionPerformed(ActionEvent ae) { //Automatisch hinzugefügt Methode für denn Fall das eine Aktion ausgelöst wird
        ...
        if(ae.getSource() == this.buttonDecrypt)
        {
            ...
            if(i == 5)
            {
                JOptionPane.showMessageDialog(null, lC.gT("Error1"), lC.gT("Error2"), JOptionPane.ERROR_MESSAGE); //Popup mit Fehlermeldung
            }
            if(i == 10)
            {
                JOptionPane.showMessageDialog(null,lC.gT("Error3"),lC.gT("Error2"),JOptionPane.ERROR_MESSAGE);
                JOptionPane.showMessageDialog(null,lC.gT("Error4"),lC.gT("Error2"), JOptionPane.ERROR_MESSAGE);
        ...
        if(ae.getSource() == this.buttonPasswort) {
            ...
            Passwort.setText(pF.passwortFrame1(lC.getActualLanguage(), "Text"));
        }
